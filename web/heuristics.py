...
def hyperbolicity(G, niters, degree):
    ...
    return(total_dist_sum/('''nx.diameter(G)*'''niters*((degree*(degree-1))/2)))
...
if __name__ == '__main__':
    ...
    # print(hyperbolicity(rw_G, 100, 4))
    ...
    for t in thresholds:
        ...
        rw_G = make_rw_graph(path)
        print("with threshold {}, branch score is {}".format(t, hyperbolicity(rw_G, 100, 4)))
        print ("with threshold {}, loop score is {}".format(t, loops(path)))
        loop_vals.append(loops(path))

    # # dummy data for branching values
    # branch_vals = [0.9, 0.5, 0.2, 0.1, 0.05, 0]
    #
    # plt.plot(thresholds, loop_vals)
    # plt.plot(thresholds, branch_vals)
    # plt.show()
